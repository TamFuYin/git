...
struct poly
{
	...
	void NTT(bool opt=1){
		...
		for(int m=2;m<=n;m<<=1){
			...
			for(int i=0;i<n;i+=m){
				int w=1;
				for(int j=i;j<i+(m>>1);j++,w=(ll)w*wm%MOD){
					int t=(ll)a[j+(m>>1)]*w%MOD;
					a[j+(m>>1)]=(a[j]-t+MOD)%MOD;
					a[j]=(a[j]+t)%MOD;
				}
			}
		}
	...
	void ln(const poly& f){
		int *g=newp();poly(g,n).diff(f);
		inv(f);*this*=poly(g,n);inte();del();
	}
	void ln(){ln(*this);}
...
}using fastwrite::write;using fastwrite::space;using fastwrite::endl;using fastwrite::show;
const int N=1e7+10;
int a[N];
int main(){

...
std::map<frac,int> mp;
std::map<std::pair<frac,frac>,int> mp2;
...
int main(){
	...
	for(int i=1;i<=n;i++){
		for(int j=i+1;j<=n;j++){
			frac k(y[j]-y[i],x[j]-x[i]);
			++mp[k];
			frac b=-k*x[i]+y[i];
//			printf("%d~%d %d/%d %d/%d\n",i,j,k.x,k.y,b.x,b.y);
			++mp2[std::make_pair(k,b)];
		}
	}
	ll ans=0;
	for(auto p:mp) ans+=binom(p.second,2);
	for(auto p:mp2) ans-=binom(p.second,2);
	printf("%lld",ans);
}

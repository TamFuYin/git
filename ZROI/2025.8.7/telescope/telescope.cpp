...
void preproc(){
	...
	inv[1]=1;
	for(int i=2;i<MAXN;i++) inv[i]=Mul(MOD-MOD/i,inv[MOD%i]),assert(Mul(inv[i],i)==1);
}
//int r[MAXN],G[MAXN*2];
//void DFTinit(){
...
int main(){
	...
	f[1]=1;
	for(int i=2;i<M;i++){
		if(!np[i]) p.push_back(i),f[i]=n,mp[i]=1;
		for(int j:p){
			if(1ll*i*j>=M) break;
			np[i*j]=1;
			if(i%j==0){
				f[i*j]=Mul(Mul(f[i],inv[mp[i]+1]),mp[i]+n);
				mp[i*j]=mp[i]+1;
				break;
			}
			mp[i*j]=1;

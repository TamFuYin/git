...
int cnt=1,n;
void solve4(int j=0){
	for(int i=a[3].size()-1;i>=0;i--){
		if(j==a[2].size()) break;
		if((ll)a[3][i]*a[2][j]>n) continue;
		++cnt,++j;
		if(j==a[2].size()) break;
		if((ll)a[3][i]*a[2][j]>n) continue;
		++cnt,++j;
...
void solve5(){
	int j=0,i;
	for(i=a[4].size()-1;i>=0;i--){
		if(j==a[2].size()) break;
		if((ll)a[4][i]*a[2][j]>n) continue;
		++cnt,++j;
		if(j==a[2].size()) break;
		if((ll)a[4][i]*a[2][j]>n) continue;
		++cnt,++j;
	...
	if(j!=a[2].size()) solve4(j);
	else{
		j=0;
		std::fill(cur,cur+a[3].size(),2);
		for(;i>=0;i--){
			for(int k=j;k<j+3&&k<a[3].size();k++){
				if((ll)a[4][i]*a[3][k]>n) break;
				--cur[k];++cnt;
			}
			while(!cur[j]){
				++j;
		...
		int tot=a[3].size()-j,cur1=0;
		while(j<a[3].size()&&cur[j]==1) ++j,++cur1;
		int cur2=tot-cur1;
		if(cur2>2||cur2==0) cnt+=cur2+cur1/2;
		else if(cur2==2){
			if(cur1>=2) cnt+=(cur1-2)/2+3;
			else cnt+=cur1+1;
		}
		else{
			if(cur1>=2) cnt+=(cur1-2)/2+2;
			else cnt+=cur1;
		}
	}
}
